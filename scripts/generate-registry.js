...
const outputFile = path.join(process.cwd(), "registry/index.js");
...
`;
    for (const folder of REGISTRY_FOLDERS) {
        ...
        const files = fs
            ...
            .filter((f) => f.endsWith(".jsx") || f.endsWith(".js") || f.endsWith(".tsx") || f.endsWith(".ts"));
        for (const file of files) {
            const name = toComponentName(file);
            const baseName = file.replace(/\.(jsx|js|tsx|ts)$/, "");
  ...
  "utf-8"
    ...
    exports += "};\n";
    const finalContent = componentImports + "\n" + fsImports + sourceConsts + exports;
